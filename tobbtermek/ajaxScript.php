...
$check_file = 'query_progress.txt';
$check_time = 600; //mértékegység: (sec) ( 10 perc = 600, 1 óra = 3600)

/* ----- 
    Ha a szöveges fájl valamiért már több mint 10 perce készült,
    az nagy valószínűséggel azt jelenti, 
...
if(file_exists($check_file) && (time() - $check_time < filemtime($check_file))){
    ...
    $tmp = fopen($check_file, "w") or die("Unable to open file!");
    $txt = "1";
    fwrite($tmp, $txt);
    fclose($tmp);
}
